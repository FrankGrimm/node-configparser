...
var ConfigParser = function(defaults) {
  ...
  // table for section subscriptions
  this._subscriptions = {};

  // override on and removeListener for section specific change listeners
  this._on = this.on;
  this.on = function(type, listener) {
    
    if (type.indexOf('change#') == 0 && type.length > 7 && listener instanceof Function) {
      var section = type.substr(7);

      if (section) {
        section = this.optionxform(section);
        if (!(section in this._subscriptions)) {
          var subscr = this._subscriptions[section] = [];
          subscr.push(listener);
        } else {
          this._subscriptions[section].push(listener);
        }
      }
    } else {
      return this._on(type, listener);
    }

    return this;
  }

  this._bool_conversion = {'true': true, '1': true, 'yes': true, 'on': true,
                          'false': false, '0': false, 'no': false, 'off': false}

  this._removeListener = this.removeListener;
  this.removeListener = function(type, listener) {
    if (type.indexOf('change#') != 0) {
      return this._removeListener(type, listener);
    } else {
      var section = this.optionxform(type.substr(7));

      if (section in this._subscriptions) {
        var i = this._subscriptions[section].indexOf(listener);
        if (i >= 0) this._subscriptions[section].splice(i, 1);
      }
    }
  ...
  // declare handler for change subscriptions
  this.on('change', function(section, option, oldValue, newValue) {
    section = this.optionxform(section);
    var callbacks = this._subscriptions[section] || null;
    if (callbacks instanceof Array) {
      for (var i = 0, l = callbacks.length; i < l; i++) {
        callbacks[i].call(this, section, option, oldValue, newValue);
      }
    }
  });

  // nextTick is used to defer the emit so that listeners can be added
...
ConfigParser.prototype.parse = function(content, filename) {
  ...
  for (var i = 0, l = changes.length; i < l; i++) {
    var c = changes[i];
    this.emit('change', this.optionxform(c.sectionName), c.optionName, c.oldVal, c.newVal); 
  }
...
ConfigParser.prototype.get = function(section, option, defaultValue) {
  ...
  // current state does not have a value for this section/option combination
  // if a default value has been passed to this call, return it
  if (defaultValue !== undefined) return defaultValue;

  // check if there is a known global default
  ...
  // neither state, nor local or global default found -> raise an exception
...
ConfigParser.prototype.getint = function(section, option, defaultValue) {
  var c = this.get(section, option, defaultValue);
  return parseInt(c);
}

// return the value as float, NaN otherwise (behaviour as specified for parseFloat)
ConfigParser.prototype.getfloat = function(section, option, defaultValue) {
  var c = this.get(section, option, defaultValue);
...
ConfigParser.prototype.getboolean = function(section, option, defaultValue) {
  var c = this.get(section, option, defaultValue);
  if (typeof c === 'boolean') return c;
  if (typeof c === 'string') {
    var v = this._bool_conversion[c.toLowerCase()];
...
ConfigParser.prototype.getJSON = function(section, option, defaultValue) {
