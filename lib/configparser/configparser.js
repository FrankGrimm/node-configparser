...
ConfigParser.prototype._get_section = function(sectionName) {
  ...
  sec.push(this.optionxform(SECTION_DEFAULT));

  for(var i = 0; i < sec.length; i++) {
    var curName = this.optionxform(sec[i]);
    if (curName == sectionName) return this._state[curName];
  }
...
ConfigParser.prototype.readfile = function(filename) {
  ...
  if (filename) {
    fs.readFile(filename, 'utf-8', function(err, data) {
      if (err) {
        self.emit('error', {where: 'readfile', error: err});
        return;
      }
...
ConfigParser.prototype.getJSON = function(section, option) {
  ...
  try {
    ...
    this.emit('error', {where: 'getJSON', error: e});
  }
}
